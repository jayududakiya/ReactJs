...
export default function Register() {
  ...
  // IMAGES
  // const [avatar,setAvatar] = React.useState("https://xsgames.co/randomusers/avatar.php?g=pixel")
  ...
  // FROM SUBMIT FUNC
  const FormSubmit = (even) => {
    even.preventDefault();
    let ID = parseInt(Math.random()*100);
    ID < 10 ? ID='0'+ID : ID;
    const { name, email, job, role, date,gender ,password} = users;
    const Sub_ID =name.slice(0,3) 
    users.id = Sub_ID+"_"+ID;
    if (name === "") {
      alert("ENTER NAME INPUT FILD");
    } else if (email === "") {
      alert("ENTER EMAIL INPUT FILD");
    }else if(date === ""){
      alert("ENTER DATE INPUT FILD");
    }
     else if (job === "") {
      alert("ENTER JOB INPUT FILD");
    } else if (role === "") {
      alert("ENTER ROLE INPUT FILD");
    }else if(gender === ""){
      alert("SELECT YOUR GENDER ");
    }
    else if (password === "") {
      alert("ENTER PASSWORD INPUT FILD");
    }
    else {
      
      // DATA SET IN LOCAL STORE      
      if (!localStorage.getItem("USERS")) {
        ...
        const findUser = localData.find((x) => {
          if (x.email === users.email){
            return x;
          }
        });
        if (findUser === undefined) {
          localStorage.setItem("USERS", JSON.stringify([...localData, users]));
          Success_Alert("YOUR ARE REGISTER SUCCESSFULLY")
          setTimeout(() => {
            neviGateTable("/");
          }, 2000);
      ...
      //END OF DATA SET IN LOCAL STORE
      even.target.reset();
      setUsers((prevVal) => {
      return {
        ...prevVal,
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-4">
                <div className="w-full">
                  ...
                  <p className="mt-1.5 text-xs text-red-500 w-full">
